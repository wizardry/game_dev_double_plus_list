...
export const getters = {
  getGenreById: (state) => (id) => {
    return state.genres.find((g) => (id == g.id));
  },
  getContentById: (state) => (id) => {
    return state.contents.find((c) => (id == c.id));
  },
  getJobById: (state) => (id) => {
    return state.jobs.find((j) => (id == j.id));
  },
  getTrainingById: (state) => (id) => {
    return state.trainings.find((t) => (id == t.id));
  },
  ...
  filterdGenres(state, getters, rootState) {
    const result = releaseGenre.filter((genre) => {
      console.log(genre, rootState.filter.selectedJob)
